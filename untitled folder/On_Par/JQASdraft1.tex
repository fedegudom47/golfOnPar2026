...
\begin{figure}[htbp]
    ...
    \caption{Simulated outcomes for a single shot type (Hybrid, $N=300$) at two aimpoints, $\aimpoint=+10$y (top) and $\aimpoint=-10$y (bottom), sharing one colour scale. Each landing is coloured by its expected strokes to hole out, showing how the same dispersion pattern is scored differently depending on where it lands relative to the green and hazards.}
    \label{fig:both}
\end{figure}
